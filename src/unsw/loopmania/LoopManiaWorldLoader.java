...
import unsw.loopmania.Buildings.CastleHero;
...
public abstract class LoopManiaWorldLoader {
    ...
    public LoopManiaWorld load() {
        ...
        // extracts the goals from the json file and converts them to objects of the goal class
        JSONObject goal = json.getJSONObject("goal-condition");
        Goal goals = convertGoals(goal);
    ...
    public Goal convertGoals(JSONObject goal) {
        Goal newGoal;
        String goalType = goal.getString("goal");

        if (goalType.equals("AND")) {
            newGoal = new ComplexGoal("AND");
            JSONArray and = goal.getJSONArray("subgoals");
            for (int i = 0; i < and.length(); i++) {
                JSONObject obj = and.getJSONObject(i);
                ((ComplexGoal)newGoal).addGoal(convertGoals(obj));
            }
        }
        else if (goalType.equals("OR"))   {
            newGoal = new ComplexGoal("OR");
            JSONArray or = goal.getJSONArray("subgoals");
            for (int i = 0; i < or.length(); i++) {
                JSONObject obj1 = or.getJSONObject(i);
                ((ComplexGoal)newGoal).addGoal(convertGoals(obj1));
            }
        }   
        else if (goalType.equals("experience"))  {
            int quantity = goal.getInt("quantity");
            newGoal = new BasicGoal(goalType, quantity);
        }
    ...
    private void loadEntity(LoopManiaWorld world, JSONObject currentJson, List<Pair<Integer, Integer>> orderedPath) {
        ...
        case "hero_castle":
            CastleHero hero_castle = new CastleHero(new SimpleIntegerProperty(x), new SimpleIntegerProperty(y));
            onLoad(hero_castle);
            entity = hero_castle;
            break;
        }
    ...
    public abstract void onLoad(CastleHero hero_castle);
